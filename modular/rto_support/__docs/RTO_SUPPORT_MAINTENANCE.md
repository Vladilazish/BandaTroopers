...
- по умолчанию RTO может держать до `2` уникальных пакетов одновременно;
- лимит слотов и время полного reset настраиваются через `Game Rule Panel`;
- reset слотов всегда полный и открывается по runtime-таймеру от первого выбора;
