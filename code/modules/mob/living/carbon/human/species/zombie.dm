...
/datum/species/zombie/handle_death(mob/living/carbon/human/zombie, gibbed)
	...
	if(zombie)
		...
		if(!QDELETED(head) && (can_rise_again(zombie)))
			if(zombie.client)
				zombie.play_screen_text("<span class='langchat_notification' style=text-align:center valign='top'><u>You are dead...</u></span><br>You will rise again in three minutes.", /atom/movable/screen/text/screen_text/command_order, rgb(155, 0, 200)) // SS220 Fonts
			...
			if(zombie.client)
				zombie.play_screen_text("<span class='langchat_notification' style=text-align:center valign='top'><u>You are dead...</u></span><br>No reviving for you.", /atom/movable/screen/text/screen_text/command_order, rgb(155, 0, 200)) // SS220 Fonts
...
/datum/species/zombie/handle_head_loss(mob/living/carbon/human/zombie)
	...
	if(WEAKREF(zombie) in to_revive)
		...
		if(receiving_client)
			receiving_client.mob.play_screen_text("<span class='langchat_notification' style=text-align:center valign='top'><u>Beheaded...</u></span><br>Your corpse will no longer rise.", /atom/movable/screen/text/screen_text/command_order, rgb(155, 0, 200)) // SS220 Fonts
...
/datum/species/zombie/proc/clean_up_zombie(mob/living/carbon/human/zombie)
	if(!zombie.zombie_disable_auto_clean)
		if(zombie.loc)
			...
			if(prob(15))
				playsound(zombie, 'sound/effects/blood_squirt.ogg', 30, TRUE)
			zombie.add_splatter_floor(zombie.loc)
			zombie.add_splatter_floor(zombie.loc)
			zombie.add_splatter_floor(zombie.loc)
			zombie.add_splatter_floor(zombie.loc)
